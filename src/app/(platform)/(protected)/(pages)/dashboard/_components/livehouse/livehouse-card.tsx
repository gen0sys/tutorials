...
import { LivehouseSelectedCard } from './livehouse-selected-card';

export const LivehouseCard = ({ event }: { event: DashboardEvent }) => {
  const PARTS = event.link.split('/');
  const link = PARTS[PARTS.length - 2];

  const { data: eventLivehouseInfo, refetch } = useQuery({
    queryKey: ['event', link],
    queryFn: () => getEvent(link),
    enabled: false,
  });
  ...
  return (
    <>
      ...
      {opened &&
        createPortal(
          <LivehouseSelectedCard
            eventTitle={event.event_name}
            parentCardRef={cardRef}
            setOpened={setOpened}
            menuOpen={menuOpen}
            eventLivehouseInfo={eventLivehouseInfo}
            event={event}
          />,
          document.body
        )}
